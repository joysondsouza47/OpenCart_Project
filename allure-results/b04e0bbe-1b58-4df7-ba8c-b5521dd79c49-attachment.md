...
```
Error: locator.click: Unexpected token "", { name: "" while parsing css selector "button', { name: 'Continue' })". Did you mean to CSS.escape it?
Call log:
  - waiting for button', { name: 'Continue' })
...
```ts
  ...
  26 |     this.btnContinue = page.locator("button', { name: 'Continue' })");

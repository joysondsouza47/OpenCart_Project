...
```
Error: locator.textContent: Unsupported token "{" while parsing css selector "('h1').filter({ hasText: 'Your Account Has Been Created!' })". Did you mean to CSS.escape it?
Call log:
  - waiting for ('h1').filter({ hasText: 'Your Account Has Been Created!' })
...
```ts
  ...
  27 |     this.msgConfirmation = page.locator("('h1').filter({ hasText: 'Your Account Has Been Created!' })");
  28 |  }
  29 | 
  30 | 
